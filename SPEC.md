...
The [general preparation milestone](docs/general-preparation.md) adds model-assisted
JSON task/rubric preparation through the native workflow. Broader semantics,
source research and live validation of that preparer remain outstanding.
...
**Implemented general JSON preparation, 2026-09-09.** Version 2 preparation
requests accept ordinary descriptions and optional supplied sources. Reusable
Inspect task/case preparation calls and an independent review produce a shared
expert/guided proposal, verified witnesses and calibration, then the same native
v2 definition used for freezing, execution, scoring and reporting. Questions,
assumptions, source originals, split lineage, model responses, failed attempts
and revisions remain inspectable. No new per-domain generator or checker is
needed within the declarative JSON predicate contract. This is bounded coverage:
semantic judgment, external dataset research, broader inference and live
preparation quality remain unimplemented or unverified. The
[general guide](docs/general-preparation.md) distinguishes product operations,
test inputs/adapters and acceptance scope.
...
| --- | --- | --- |
| Y01 — Adaptive preparation ([section 3](#3-helping-the-user-define-a-study)) | **Partial, expanded.** [General preparation](src/yassa/general_preparation.py) accepts user-described JSON work, focused answer rounds, model proposals and independent reviews beyond fixed recipes; expert imports share compilation. A bounded native live acceptance validates synthesis/review on one room-booking task, including a preserved failed review and correction. | A callable preparation workflow develops user-described work beyond those recipes into the same definition as the expert path; sufficient requests proceed, material gaps get focused questions, and assumptions and revisions remain inspectable. The developer must not manually author a replacement study for the demonstration. |
| Y02 — Tasks, rubrics and materials ([sections 3](#3-helping-the-user-define-a-study), [6](#6-benchmark-templates-and-task-selection)) | **Partial, expanded.** [Declarative rubrics](src/yassa/json_rubric.py) and reusable case preparation add predicate-verified witnesses, positive/negative calibration, raw supplied sources, adaptation lineage and constructed cases. Remote source research and broader semantic validation remain missing. | Reusable rubric and case-preparation operations support the selected wider task contracts, source research/adaptation when needed, legitimate alternatives, verification of references, grouping and held-out splits. Exercise supplied and prepared material beyond a new hardcoded reconciliation recipe; retain source/access and synthetic limitations. |
| Y03 — Study definitions and comparisons ([sections 4](#4-study-definition), [5](#5-comparisons-and-experimental-conditions), [11](#11-inspect-integration-requirements)) | **Partial, expanded.** General preparation connects to native builder/consumer studies. [Direct studies](docs/execution-roles.md) reuse pinned tasks and both material routes with native or closed API roles, explicit treatments and zero build parents. New direct roles have software acceptance; live role validation remains separate. | Generalize the resolved definition and real execution routes for the selected direct and builder study forms, role/model assignments and supported task evidence. Preserve preparation conditions and declared treatment differences; explicitly resolve activation/clarification conditions when selected. Verify compatibility with recorded v1/v2 evidence. |
| Y04 — Sources and access boundaries ([sections 2](#2-scope-and-commitments), [9](#9-execution-resource-accounting-and-failure-handling), [11](#11-inspect-integration-requirements)) | **Partial, expanded.** Native preparation/direct work reuse pinned packs, Docker isolation and catalog gates. Closed API direct/grading roles expose explicit messages without tools and retain source/output hashes. Software tests check held-out inputs and grader label withholding. | Resolve required sources/dependencies and enforce declared filesystem, configuration, tool and network boundaries on every supported route. Record each adapter's observed limits; the current child postflight is not per-request interception or upstream attestation. Acceptance includes protected-context checks on added routes. |
| Y05 — Multiple vendor families ([sections 2](#2-scope-and-commitments), [13](#13-first-dovetail-study-decisions-still-open)) | **Partial implementation; live cross-vendor evidence missing.** Direct execution and external grading support first-party OpenAI/GPT, Anthropic/Claude and Google/Gemini Inspect roles with pinned SDKs. Their real provider classes construct, and simulated calls test orchestration; additional native hosts and API builders remain open. | Implement and separately verify the additional provider/runtime roles needed for a study program involving different vendors. Record the role and capabilities of each family and host; do not attribute host differences solely to the skill builder. Exact vendor/model selections remain study decisions. |
...
1. **General preparation through a runnable study (Y01, Y02, Y03, Y10).** Extend
   ...
   this milestone. The bounded JSON implementation now has offline connected
   evidence and a separately scoped native live room-booking acceptance. General
   preparation reliability and full capability breadth remain open. See
   [its scope](docs/general-preparation.md).
2. **Execution and grading across the required roles (Y03, Y04, Y05, Y07).** Add
   ...
   required by the selected wider contracts. Direct native/API execution and
   external calibration/grading now advance this step through supported commands.
   Additional native hosts, builder roles and live cross-vendor verification remain
   open. Preserve existing execution and evidence boundaries; verify each newly
   claimed native capability separately. Extend resource/recovery controls in
   step 3 alongside these effectful roles before larger allocations.
